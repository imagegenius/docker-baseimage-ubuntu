  ...
  stages {
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              # ${TEMPDIR}/docker-${CONTAINER_NAME}: Cloned branch jammy of ${IG_USER}/${IG_REPO} for running the jenkins builder on
              # ${TEMPDIR}/repo/${IG_REPO}: Cloned branch jammy of ${IG_USER}/${IG_REPO} for commiting various templated file changes and pushing back to Github
              # ${TEMPDIR}/docs/docker-documentation: Cloned docs repo for pushing docs updates to Github
              # ${TEMPDIR}/unraid/docker-templates: Cloned docker-templates repo to check for logos
              # ${TEMPDIR}/unraid/templates: Cloned templates repo for commiting unraid template changes and pushing back to Github
              git clone --branch jammy --depth 1 https://ImageGeniusCI:${GITHUB_TOKEN}@github.com/${IG_USER}/${IG_REPO}.git ${TEMPDIR}/docker-${CONTAINER_NAME}
              ...
              if [[ "$(md5sum Jenkinsfile | awk '{ print $1 }')" != "$(md5sum ${TEMPDIR}/docker-${CONTAINER_NAME}/Jenkinsfile | awk '{ print $1 }')" ]]; then
                ...
                git pull https://ImageGeniusCI:${GITHUB_TOKEN}@github.com/${IG_USER}/${IG_REPO}.git jammy
                git push https://ImageGeniusCI:${GITHUB_TOKEN}@github.com/${IG_USER}/${IG_REPO}.git jammy
      ...
      steps {
        retry(5) {
          sh '''#! /bin/bash
                ...
                docker buildx imagetools create -t ${GITHUBIMAGE}:jammy ${GITHUBIMAGE}:amd64-jammy ${GITHUBIMAGE}:arm64v8-jammy
    ...
    stage('Github-Release-Branch-Protection') {
      ...
      steps {
        ...
        sh '''#! /bin/bash
          curl -H "Authorization: token ${GITHUB_TOKEN}" -X PUT https://api.github.com/repos/${IG_USER}/${IG_REPO}/branches/jammy/protection \
          -d $(jq -c .  << EOF
            {
